...
void InterfaceTool::writeTextBufferWithMenu(WINDOW *_window, std::string buffer, screenCoords_t coords)
{
  int x = coords.tool_origin_x_menu_active + 1;
  int y = coords.tool_origin_y_menu_active + 1;
  int bufferMaxlength = coords.size_x - 18; //var '18' should be a dynamic value of tool
  ...
  if (lastBufferSize == bufferMaxlength) {
    ...
    mvwprintw(_window, ++y, x + lastBufferSize, " ");
  }
  //std::vector<int> newPos {x, y};
  //this->_currentTool->setCursor(newPos);
  wattroff(_window, COLOR_PAIR(14));
}

void InterfaceTool::writeTextBufferWithoutMenu(WINDOW *_window, std::string buffer, screenCoords_t coords)
{
...
void InterfaceTool::writeCursor(WINDOW *_window, std::vector<int> pos)
{
    wattron(_window, COLOR_PAIR(17));
    mvwprintw(_window, pos.at(0), pos.at(1), "@");
    wattroff(_window, COLOR_PAIR(17));
    return;
}
                ...
                if (this->_tools[it] == this->_currentTool) {
                    switch ((int) this->_screenSetup) {
                        ...
                        case InterfaceTool::ScreenSetup::WIDE:
                            ...
                            writeTextBufferWithMenu(_window, this->_currentTool->getBuffer(), _coords);
                            writeCursor(_window, this->_currentTool->getCursorCurrentPosition());
                            break;
                    }
                } else {
                    switch ((int) this->_screenSetup) {
...
{
    Tool *notepad = new Tool("NOTEPAD", this->_coords);
    Tool *reminder = new Tool("REMINDER", this->_coords);
    Tool *calendar = new Tool("CALENDAR", this->_coords);
...
int InterfaceTool::handleInputsTyping(Keys::Key event)
{
    ...
    switch ((int) event) {
        case Keys::K_UP:
            this->_currentTool->moveCursorUp();
            return (EX_OK);
            break;
        case Keys::K_CLOSE:
        case Keys::K_DOWN:
...
int InterfaceTool::render(WINDOW *_window)
{
    ...
    if (this->_keyboardMode == InterfaceTool::KeyboardScope::TYPING) {
        mvwprintw(_window, (COLS - COLS + 25), (LINES - 9) , "TYPING");
        if (this->_menu->getToggle() == true) {
            if (this->_currentTool == nullptr) {
        ...
        mvwprintw(_window, (COLS - COLS + 25),(LINES - 9), "NAVIGATION");
        if (this->_menu->getToggle() == true) {
            if (this->_currentTool == nullptr) {
